...
class FavoritesScreen extends StatefulWidget {
  ...
  const FavoritesScreen({super.key, required this.musicService});
...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  List<MusicTrack> _getFavoriteSongs() {
    return _songs.where((s) => _favorites.contains(s.id)).toList();
  }

  @override
  Widget build(BuildContext context) {
    final favSongs = _getFavoriteSongs();
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.favorite, color: Color(0xFFEFBBFF), size: 28),
